...
class Webpacker
{
  ...
  public function addAsset($fileExt, $baseName, $tag = false, $attr = null)
  {
    ...
    // Manifest path
    $manifestPath = URL::assemble($themeRootPath, $this->outputFolder, $manifestName);
    ...
    if (is_resource($devServer) && File::exists($hotPath)) {
      ...
      // Manifest data as array
      $manifest = json_decode(File::get($manifestPath), true);

      // Stop here if asset isn't in wepacker.json
      if (empty($manifest[$fileName])) {
        if ($fileName === 'vendor.js' || $fileName === 'common.js') {
          return;
        } else {
          throw new \RuntimeException("{$fileName} isn't in {$manifestName}, please run webpacker:prod script from your theme folder again");
        }
      }

      // Asset path
      $assetPath = $manifest[$fileName];

      // Stop here if asset file doesn't exist
      if (!file_exists(root_path($assetPath))) {
        // Trow missing file error
        throw new \RuntimeException("{$fileName} is in {$manifestName} but the file can't be found, please run webpacker:prod script from your theme folder again");
      }

      // Check if static cache is enabled
      if (Config::get('caching.static_caching_enabled')) {
        // Get manifest build date
        $manifestDate = strtotime($manifest['BUILD_INFORMATIONS']['date']);

        // Check if manifest have change since last static cache
        if ($manifestDate != $this->cache->get($manifestName)) {
          // Clear static cache
          Please::call('clear:static');
          // Save new manifest date to the cache for next time
          $this->cache->put($manifestName, $manifestDate);
        }
      }
      ...
      throw new \RuntimeException("{$manifestName} doesn't exist and developement server is not active. Please run webpacker:prod or webpacker:dev script from your theme folder");
    }
  }
  ...
  private function addJs($assetPath, $tag, $attr)
  {
    if ($tag === true) {
      ...
      return "<script charset=\"utf8\" src=\"{$assetPath}\" {$attributes}></script>";
    } elseif ($tag === 'inline') {
